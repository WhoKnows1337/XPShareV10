...
export function LiveInsightsSidebar({
  ...
  // Map backend category slugs to display info
  const getCategoryInfo = (slug: string | null): { label: string; emoji: string } | null => {
    if (!slug) return null
    
    // Map backend slugs to display labels
    const categoryMap: Record<string, { label: string; emoji: string }> = {
      'ufo-sighting': { label: 'UFO Sighting', emoji: '🛸' },
      'entity-encounter': { label: 'Entity Encounter', emoji: '👽' },
      'paranormal-activity': { label: 'Paranormal', emoji: '👻' },
      'time-anomaly': { label: 'Time Anomaly', emoji: '⏰' },
      'precognition': { label: 'Precognition', emoji: '🔮' },
      'telekinesis': { label: 'Telekinesis', emoji: '🌀' },
      'telepathy': { label: 'Telepathy', emoji: '🧠' },
      'astral-projection': { label: 'Astral Projection', emoji: '✨' },
      'cryptid-encounter': { label: 'Cryptid', emoji: '🦎' },
      'dimensional-shift': { label: 'Dimensional Shift', emoji: '🌌' },
      'lucid-dreams': { label: 'Lucid Dreams', emoji: '💭' },
      'nightmares': { label: 'Nightmares', emoji: '😱' },
      'psychedelic-experience': { label: 'Psychedelic', emoji: '🌈' },
      'spiritual-awakening': { label: 'Spiritual', emoji: '🙏' },
      'synchronicity': { label: 'Synchronicity', emoji: '🔗' },
      'near-death': { label: 'Near-Death Experience', emoji: '💫' },
      'other': { label: 'Other Experience', emoji: '🌟' },
    }
    
    return categoryMap[slug] || { label: slug, emoji: '📌' }
  }

  const categoryInfo = getCategoryInfo(analysis?.category || null)
